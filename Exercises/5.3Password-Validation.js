...
function simplePassword2(password){
    let passwordStrength = password.length > 7 ? "Strong Password" : "Weak Password";
...
function advancedPassword(password){
    let capRegEx = /[A-Z]/g
    let passwordStrength = password.length >= 7 && capRegEx.test(password) ? "Very Strong Password" : password.length >= 7 ? "Strong Password" : "Weak Password" ;
    return passwordStrength;
